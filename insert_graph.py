...
with PdfPages('tree_performance_final.pdf') as pdf:
    ...
    ax = axes[2]
    ax.plot(df_insert['order'],df_insert['B_split'],color=colors['B'],label=labels['B'],linewidth=1.5)
    ax.plot(df_insert['order'],df_insert['BPlus_split'],color=colors['BPlus'],label=labels['BPlus'],linewidth=1.5)
    ax.plot(df_insert['order'],df_insert['BStar_split'],color=colors['BStar'],label=labels['BStar'],linewidth=1.5)
    ...
    fig, axes = plt.subplots(5, 1, figsize=(10, 25), dpi=300)
    fig.subplots_adjust(hspace=0.45)

    ax = axes[0]
    ax.plot(df_point['order'],df_point['B_time'],color=colors['B'],linewidth=1.0,alpha=0.15)
    ax.plot(df_point['order'],df_point['BPlus_time'],color=colors['BPlus'],linewidth=1.0,alpha=0.15)
    ax.plot(df_point['order'],df_point['BStar_time'],color=colors['BStar'],linewidth=1.0,alpha=0.15)
    ax.plot(df_point['order'],df_point['B_time'].rolling(window=window_size).mean(),color=colors['B'],label=labels['B'],linewidth=2.0,alpha=1.0)
    ax.plot(df_point['order'],df_point['BPlus_time'].rolling(window=window_size).mean(),color=colors['BPlus'],label=labels['BPlus'],linewidth=2.0,alpha=1.0)
    ax.plot(df_point['order'],df_point['BStar_time'].rolling(window=window_size).mean(),color=colors['BStar'],label=labels['BStar'],linewidth=2.0,alpha=1.0)
    ax.set_title('Point Search Execution Time',fontsize=14,fontweight='bold')
    ax.set_xlabel('Order (d)',fontsize=12)
    ...
    ax = axes[1]
    ax.plot(df_range['order'],df_range['B_time'],color=colors['B'],label=labels['B'],linewidth=1.0,alpha=0.8)
    ax.plot(df_range['order'],df_range['BPlus_time'],color=colors['BPlus'],label=labels['BPlus'],linewidth=1.0,alpha=0.8)
    ax.plot(df_range['order'],df_range['BStar_time'],color=colors['BStar'],label=labels['BStar'],linewidth=1.0,alpha=0.8)
    ax.set_title('Range Search Execution Time',fontsize=14,fontweight='bold')
    ax.set_xlabel('Order (d)',fontsize=12)
    ax.set_ylabel('Average Execution Time(ns)',fontsize=12)
    ax.grid(True, linestyle='--',alpha=0.5)
    ax.legend(fontsize=11)
    ...
    ax = axes[2]
    ax.plot(df_delete['order'],df_delete['B_time'],color=colors['B'],linewidth=1.0,alpha=0.15)
    ax.plot(df_delete['order'],df_delete['BPlus_time'],color=colors['BPlus'],linewidth=1.0,alpha=0.15)
    ax.plot(df_delete['order'],df_delete['BStar_time'],color=colors['BStar'],linewidth=1.0,alpha=0.15)
    ax.plot(df_delete['order'],df_delete['B_time'].rolling(window=window_size).mean(),color=colors['B'],label=labels['B'],linewidth=2.0,alpha=1.0)
    ax.plot(df_delete['order'],df_delete['BPlus_time'].rolling(window=window_size).mean(),color=colors['BPlus'],label=labels['BPlus'],linewidth=2.0,alpha=1.0)
    ax.plot(df_delete['order'],df_delete['BStar_time'].rolling(window=window_size).mean(),color=colors['BStar'],label=labels['BStar'],linewidth=2.0,alpha=1.0)
    ax.set_title('Deletion Search Execution Time',fontsize=14,fontweight='bold')
    ax.set_xlabel('Order (d)',fontsize=12)
    ax.set_ylabel('Average Execution Time(ns)',fontsize=12)
    ax.grid(True, linestyle='--',alpha=0.5)
    ax.legend(fontsize=11)
    ...
    ax.set_title('Merge Count (d = 4 to 100)',fontsize=14,fontweight='bold')
    ax.set_xlabel('Order (d)',fontsize=12)
